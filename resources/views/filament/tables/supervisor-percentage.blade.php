<div class="ml-3">
    @php
        ...
        $student = $getRecord()->student ?? null;
        $responses = $student ? \App\Models\RecommendationResponse::where('student_id', $student->id)->first() : null;
        $resume = \App\Models\Resume::where('user_id', $getRecord()->student->user->id)->first();

        $percentage = 0;

        $supervisor = \App\Models\Supervisor::where('user_id', auth()->id())->first();

        // @dump($getRecord()->student->id);

        if ($responses) {
            try {
                // Decode job requirements with error handling
                $data = json_decode($supervisor->job_requirements);
                ...
                // 4. Hard Skills (array comparison)

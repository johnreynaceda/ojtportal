<div x-data>
    ...
    @else
        @if ($edit_resume)
            ...
            <div class="mt-5 flex space-x-3 items-center">
                <x-button label="Submit Record" negative squared wire:click="updateResume" spinner="submitRecord" />
                <x-button label="Close" href="{{ route('student.dashboard') }}" slate squared />
            </div>
        @else
            <div class="flex justify-end space-x-4">
                <x-button label="EDIT" class="font-semibold" positive wire:click="edit" />
                ...
                <div class="mt-20 grid grid-cols-2 px-10 gap-5">
                    <div class=" flex flex-col space-y-10">
                        ...
                        <div>
                            ...
                            <div class="mt-2 h-1 w-40 bg-yellow-800"></div>
                            <div class="mt-5 grid-cols-3 grid gap-1">
                        ...
                        <div>
                            ...
                            <div class="mt-5 grid-cols-3 grid gap-1">
                                @php
                                    $soft = json_decode(auth()->user()->resume->soft_skill, true);
                                @endphp
                        ...
                        <div>
                            ...
                            @if (auth()->user()->resume->education_background != 'null')
                                <h1 class="text-3xl font-bold text-yellow-600">Education Background</h1>
                                <div class="mt-2 h-1 w-40 bg-yellow-800"></div>
                                <ul class="mt-5 space-y-2">
                                    @php
                                        $educ = json_decode(auth()->user()->resume->education_background, true);
                                    @endphp
                    ...
                    <div>
                        <div class="flex flex-col space-y-10">
                            ...
                            <div>
                                @if (auth()->user()->resume->work_experience != 'null')
                            ...
                            <div>
                                ...
                                @if (auth()->user()->resume->award != 'null')
                                    <h1 class="text-3xl font-bold text-yellow-600">Awards</h1>
                                    <div class="mt-2 h-1 w-40 bg-yellow-800"></div>
                                    <ul class="mt-5 space-y-2">
                                        @php
                                            $award = json_decode(auth()->user()->resume->award, true);
                                        @endphp
                            ...
                            <div>
                                @if (auth()->user()->resume->certification != 'null')
                                    <h1 class="text-3xl font-bold text-yellow-600">Certifications</h1>
                                    <div class="mt-2 h-1 w-40 bg-yellow-800"></div>
                                    <ul class="mt-5 space-y-2">
                                        @php
                                            $cert = json_decode(auth()->user()->resume->certification, true);
                                        @endphp

...
class CoordinatorChat extends Component implements HasForms
{
   ...
   $this->user_data = User::where('id', $this->user_id)->first();
    ...
    public function chatUser($id){
        $data = Chat::whereJsonContains('user_ids', $id)->first();
        if ($data) {
            dd('meron');
        }else{
           $chat =  Chat::create([
    ...
    {
        return view('livewire.coordinator-chat',[
            'users' => User::where('id', '!=', auth()->user()->id)->where('name', 'like', '%'. $this->search. '%')->get(),
            'chats' => Chat::whereJsonContains('user_ids', auth()->user()->id)->get(),
        ]);
    }
}
